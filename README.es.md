...
1. Inicia la app y elige **Probar con una pista generada**. No necesitas archivos ni cuenta.
2. Pulsa `H` para dejar toda la pantalla a la escena y `R` para explorar otra vibra.
3. Pulsa `V` para combinar capas o `G` para observar los medidores de **Audio sync**.
4. Arrastra tu propia canción para descubrir cómo cambia la respuesta con otro arreglo.

<table>
  <tr>
    <td width="33%"><strong>Siente el ritmo</strong><br>Acentos de percusión, partículas e impactos sincronizados.</td>
    <td width="33%"><strong>Mira la armonía</strong><br>Cintas, filamentos y color que responden al contenido tonal.</td>
    <td width="33%"><strong>Hazlo tuyo</strong><br>Trece capas, materiales físicos, paletas y una vibra que se guarda.</td>
  </tr>
</table>

<p align="center">
  <img src="docs/screenshots/07-audio-sync.png" alt="Escena real de cristal lavanda y azul reaccionando a la pista sintetizada" width="100%">
  <br><em>Una captura real de la demo: el movimiento y el color evolucionan con el sonido.</em>
</p>
...
ritmo ambiental y un acorde que rota) para que ninguna capa se quede plana esperando datos que no van a
llegar.
...
Un `AudioWorklet` analiza los canales estéreo por separado en ventanas de unos 8 ms,
independientemente de los FPS. Una cola acotada sincroniza ataques de bombo, caja y hi-hat,
une duplicados estéreo y descarta impactos atrasados. Las pausas, saltos y cambios de canción
reinician el análisis. Si AudioWorklet no está disponible, se utiliza el análisis por frame.

El director visual distribuye acentos y movimiento continuo según estimaciones armónicas,
sostenidas y percusivas. Son **estimaciones de señal, no reconocimiento entrenado de
instrumentos, géneros o emociones**. El tempo requiere suficiente confianza; la animación
ambiental no muestra un BPM medido. La tarjeta **Audio sync** en **Ajustes → Visual** muestra
estas estimaciones. Consulta las [notas técnicas](docs/audio-sync.md) y las
[pruebas con Spotify](docs/spotify-listening-tests.md).

Como complemento, de un `AnalyserNode` con FFT de 4096 (≈11,7 Hz por bin a 48 kHz) sale:
...
| **Resonancia** | Acentos de impacto con tiempo de audio y doce filamentos armónicos |
...
```
escena → protección HDR + compresión de altas luces → estelas → bloom → pase de imagen → salida
```

La protección HDR elimina valores inválidos antes de que el bloom los propague por toda la
pantalla. Las [pruebas de regresión](docs/render-blackout.md) documentan la corrección del apagón.
...
1. **Análisis de audio de Spotify** (sin capturar nada). Al cambiar de canción se descarga
   ...
   > El acceso a este endpoint está restringido. Si no está disponible para tu app,
   > utiliza una de las opciones de captura de audio real.
...
2. **Salida del sistema como entrada de audio**. Si el equipo expone la mezcla de salida como
   ...
   dispositivo guardado puede reconectarse si los permisos del navegador lo permiten.
   Botón **Buscar** en Ajustes → Spotify.
...
src/
  audio/
    ...
    sync-dsp.js     detección adaptativa de transitorios por muestras
    sync-worklet.js análisis estéreo independiente del render
    sync.js         cola de eventos, sincronización y carácter musical
  visual/
    vibe.js         generador de "personalidad" por canción
    director.js     distribución de roles musicales entre capas
    ...
    layers/         escultura, nebulosa, auroras, metaformas, flujo, partículas, anillos,
                    cintas, túnel, superficie, orbes, formas, resonancia
  spotify/
    ...
    discontinuity.js detección de cambios de canción, reproducción y saltos
...
La suite cubre audio sync, dirección visual y cambios de reproducción de Spotify.
Con `npm run dev` activo, abre el
[laboratorio de audio](http://127.0.0.1:5173/tests/audio-lab.html) para probar ritmos, acordes
y estéreo en contrafase a distintos FPS, o el
[laboratorio de render](http://127.0.0.1:5173/tests/render-safety.html) para verificar HDR y bloom.
Las notas técnicas enlazadas describen resultados y limitaciones.
...
El README está disponible en [inglés](README.md) y español; mantén ambas versiones al
documentar funciones. La interfaz y la mayoría de los comentarios están en español;
los identificadores, el historial de git, las issues y los pull requests van en inglés, siguiendo [Conventional
